...
export async function getCourseforHomePage(keyword : string) {
    let names : Name[] | undefined;
    let webcoursesSet = await getCachedCoursesForHomepage(keyword);
    if(webcoursesSet)
    webcoursesSet.forEach(function(_item: any){
console.log(_item);
     let _n : Name | undefined;
     if (_n !== undefined) {
        _n.course = _item.attributes.course_title;
        _n.imageSrc = _item.attributes.course_logo;
        _n.profession = "Engineer";
        _n.price = _item.attributes.course_fee;
        _n.category = "webdevelopment";
        console.log(_n);
        if(names !== undefined ) names.push(_n);
      }
    });
    console.log(names);
...
export async function getCachedCoursesForHomepage(metadata : string) {
    ...
    await fetch(endpointUrl, { next: { revalidate: 3600 } })
      .then(async (response) => {
        if (response.status === 200) {
          let _coursesData = await response.json();
          console.log(_coursesData);
          coursesData = _coursesData.data;
        }
      })
...
const NamesList = async () => {
    ...
    const webDevelopment = await getCourseforHomePage("webdevelopment");
    const mobileDevelopment = await getCourseforHomePage("mobileDevelopment");
    const dataScience = await getCourseforHomePage("dataScience");
    const cloudComputing = await getCourseforHomePage("cloudComputing")
  //  const mobileDevelopment = names.filter((name: { category: string; }) => name.category === 'mobiledevelopment');
 //   const webDevelopment = names.filter((name: { category: string; }) => name.category === 'webdevelopment');
 //   const dataScience = names.filter((name: { category: string; }) => name.category === 'datascience');
 //   const cloudComputing = names.filter((name: { category: string; }) => name.category === 'cloudcomputing');

    let selectedNames: Name[] = [];

    if (selectedButton === 'mobiledevelopment') {
        selectedNames = null !== mobileDevelopment ? mobileDevelopment:[];
    } else if (selectedButton === 'webdevelopment') {
        selectedNames = null !== webDevelopment ? webDevelopment:[];
    } else if (selectedButton === 'datascience') {
        selectedNames = null!== dataScience  ? dataScience : [];
    } else if (selectedButton === 'cloudcomputing') {
        selectedNames = null !== cloudComputing ? cloudComputing: []
    }
